...
assert.ok(americanEnglish.list.every(article => article.contentLanguage === "en" && !/[가-힣]/.test(article.title + (article.summary ?? ""))));
assert.deepEqual(americanEnglish.list.map(article => article.articleUrl), american.list.map(article => article.articleUrl));
...
assert.ok(english.list.every(article => article.contentLanguage === "en" && !/[가-힣]/.test(article.title + (article.summary ?? ""))));
