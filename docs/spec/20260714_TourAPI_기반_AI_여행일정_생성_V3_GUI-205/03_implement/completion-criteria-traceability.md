...
실제 양양 → 거제 외부 API 생성과 실제 Upstash 저장은 승인·호출량·비용 확인이 필요한 별도 검증이다. 실행하지 않았고 통과로 표시하지 않는다.
...
| --- | --- | --- | --- |
| 양양 → 거제 1박 2일 실제 MCP 생성 | 별도 승인 필요 | 실제 TourAPI·OpenAI·Naver·ODsay와 개발 키로 검증해야 한다. 로컬 fixture 성공을 실제 공급자 성공으로 대체하지 않는다. | 미실행 |
...
- 실제 양양 → 거제 공급자 조합은 키·호출량·비용 승인 뒤에만 확인할 수 있다.
...
승인 없이 실행할 수 있는 적용 기준과 V3 대체 기준에는 자동 증거가 있다. 외부 API·실제 Upstash 항목은 별도 승인 필요 상태로 남기며 통과로 주장하지 않는다.
