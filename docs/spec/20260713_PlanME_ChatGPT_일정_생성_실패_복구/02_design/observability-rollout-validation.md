...
- 운영 로그에는 응답 크기, 실패 단계, 내부 오류 코드, 경로 제공자, 일차, 경로 종류, 재시도 여부만 기록한다.
- 일정 본문, 장소명, 좌표, 외부 응답 원문과 인증정보는 기록하지 않는다.
...
| 경로 최종화 | 제공자, 내부 코드, 일차, 경로 종류, 재시도 여부 |

## 자동 검증

- MCP 웹 저장 요청에 유효한 추적 식별자 헤더가 존재한다.
- 맞춤형 GPT 성공 로그에 양수인 응답 바이트 수가 존재한다.
- 경로 제공자 실패 로그에 제공자, 일차, 경로 종류, 내부 오류 코드가 존재한다.
...
4. 경로 제공자 실패라면 제공자, 내부 오류 코드, 일차, 경로 종류, 재시도 여부를 확인한다.
