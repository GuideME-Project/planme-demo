...
- 주요 리스크: 오류 객체를 일반 오류로 다시 감싸는 현재 흐름에서 제공자 코드와 실패 구간이 사라질 수 있으며, 사용자 출발지·복귀지나 일정 본문이 로그에 유입되면 안 된다.
...
- 경로 제공자 오류의 제공자·내부 코드·일차·경로 종류·구간 번호·방문지와 숙소·재시도 여부 보존
- 사용자 출발지·복귀지 장소명과 좌표 제외
- MCP와 최종화 자동 테스트 보강
...
- 제공자 실패 로그에 구간 번호와 AI 방문지·숙소의 장소명·좌표가 존재한다.
- 사용자 출발지·복귀지와 테스트용 토큰 문자열은 로그에 포함되지 않는다.
...
- 사용자 출발지·복귀지를 로그에 넣어야만 진단 가능한 경우
